using _3D_Space_Colonisation_Tree;
using System;
...
namespace _3D_Space_Colonisation_Tree
{
    internal class Tree
    {
        ...
        public void SetRoot(Vector3D location, Vector3D Startdirection, double branchlength)
        {
        ...
        public List<Branch> Grow()
        {
            ...
            if (my_RootGrow == true)
            {
                ...
                {
                    d = Math.Sqrt((newBranch.Location.X - my_Leafs[I].Location.X) * (newBranch.Location.X - my_Leafs[I].Location.X) + (newBranch.Location.Y - my_Leafs[I].Location.Y) * (newBranch.Location.Y - my_Leafs[I].Location.Y) + (newBranch.Location.Z - my_Leafs[I].Location.Z) * (newBranch.Location.Z - my_Leafs[I].Location.Z));
                    if (d < my_Viewdist)
                    {
                        my_RootGrow = false;
                        break;
