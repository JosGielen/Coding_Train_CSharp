...
namespace RubiksCube.Kociemba
{
    public class Search
    {
        ...
        public static string solution(string facelets, out string info, int maxDepth = 22, long timeOut = 6000, bool useSeparator = false )
        {
            ...
            {
                info = "The input contains wrong characters (need U,D,F,B,L or R).";
                return "Error 1";
            }
            for (int i = 0; i < 6; i++)
            {
                if (count[i] != 9)
                {
                    info = "The input must contain 9 of each U,D,F,B,L or R.";
                    return "Error 1";
                }
            }
            ...
            if ((s = cc.verify()) != 0)
            {
                info = "The Cube is in an unsolveable configuration.";
