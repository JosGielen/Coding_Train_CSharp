...
namespace RubiksCube.Kociemba
{
    public class Tools
    {
        ...
        //Changed from the obsolete BinaryFormatter to MessagePack UGet package
        public static void SerializeTable(string filename, short[,] array)
        {
            ...
            MessagePackSerializer.Serialize(s, array, ContractlessStandardResolver.Options);
            s.Close();
        }
        ...
        public static short[,] DeserializeTable(string filename)
        {
            ...
            short[,] array = MessagePackSerializer.Deserialize<short[,]>(s, ContractlessStandardResolver.Options);
            s.Close();
            return array;
        }
        ...
        public static sbyte[] DeserializeSbyteArray(string filename)
        {
            EnsureFolder("Assets\\Kociemba\\Tables\\");
            Stream s = File.Open("Assets\\Kociemba\\Tables\\" + filename, FileMode.Open);
            sbyte[] array = MessagePackSerializer.Deserialize<sbyte[]>(s, ContractlessStandardResolver.Options);
            s.Close();
            return array;
        }
