...
{
    // A speed-improved simplex noise algorithm for 2D, 3D and 4D in C Sharp .Net.
    // Based on example code by Stefan Gustavson (stegu@itn.liu.se).
    // Optimisations by Peter Eastman (peastman@drizzle.stanford.edu).
    // Better rank ordering method for 4D by Stefan Gustavson in 2012.
    // Ported to C Sharp by Jos Gielen 02/03/2019
    class FastSimplexNoise
    {
        // Skewing and unskewing factors for 2, 3, and 4 dimensions
        static readonly double F2 = 0.5 * (Math.Sqrt(3.0) - 1.0);
        static readonly double G2 = (3.0 - Math.Sqrt(3.0)) / 6.0;
        static readonly double F3 = 1.0 / 3.0;
        static readonly double G3 = 1.0 / 6.0;
        static readonly double F4 = (Math.Sqrt(5.0) - 1.0) / 4.0;
        static readonly double G4 = (5.0 - Math.Sqrt(5.0)) / 20.0;
        ...
        {
            151, 160, 137, 91, 90, 5, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140,
        ...
        {
            return SmoothStep(Noise2D(x, 0), 0, 1, factor);
        }
        ...
        {
            return SmoothStep(Noise2D(x, 0, octaves, persistence), 0, 1, factor);
        }
        ...
        public static double Noise(double x, int octaves, double persistence)
        {
            ...
            if (octaves == 1) { return Noise2D(x, 0); }
            for (int I = 0; I < octaves; I++)
            {
                total += Noise2D(x * frequency , 0) * amplitude;
                maxValue += amplitude;
        ...
        /// <summary>
        /// Calculates a single octave 2D Fast Simplex Noise value between 0 and 1 for input values x,y.
        /// </summary>
        /// <param name="x">The x input of the 2D Perlin Noise function.</param>
        /// <param name="y">The y input of the 2D Perlin Noise function.</param>
        ...
        public static double Noise4D(double x, double y, double z, double w)
        {
            ...
            return (27.0 * (n0 + n1 + n2 + n3 + n4) + 1) / 2.0;
        }
