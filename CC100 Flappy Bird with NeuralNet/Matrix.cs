...
namespace Flappy_Bird_with_NeuralNet
{
    internal class Matrix : ICloneable
    {
        ...
        /// <summary>
        /// Returns a string description of a Matrix (format= [[x;x;x;..][x;x;x;..]...])
        /// </summary>
        ...
        /// <summary>
        /// Returns a formatted string description of a Matrix (format= [[x;x;x;..][x;x;x;..]...])
        /// </summary>
        /// <param name="format">specifies the format of each number in the string.</param>
        ...
        /// <para>The Format of the string must be: [[x;x;x;..][x;x;x;..]...]</para>
        /// </summary>
        public static Matrix FromString(string s)
        {
            ...
            int cols = 0;
            string[] colData;
            if (s[0] != '[' || s[s.Length - 1] != ']') FormatOK = false;
            ...
            if (rowbegins != rowstrings.Count) FormatOK = false;
            try
            {
                if (FormatOK)
                {
                    cols = rowstrings[0].Split(';').Length;
                    colData = new string[cols];
                    result = new Matrix(rowbegins, cols);
                    for (int I = 0; I < rowstrings.Count; I++)
                    {
                        colData = rowstrings[I].Split(';');
                        ...
                        {
                            result.SetValue(I, J, double.Parse(colData[J]));
                        }
                    }
                }
                else
                {
                    throw new Exception("FromString: The string has an invalid format./nCorrect Format = [[x;x;x;..][x;x;x;..]...]");
                }
            }
        ...
        /// <summary>
        /// Returns an array of strings each containing one row of the Matrix.
        /// <para>First String contains the number of Rows ; Columns</para>
        /// </summary>
        public string[] ToRowStrings()
        {
            string[] result = new string[my_rows + 1];
            result[0] = my_rows.ToString() + ";" + my_cols.ToString();
            for (int I = 0; I < my_rows; I++)
            {
                for (int J = 0; J < my_cols; J++)
                {
                    result[I + 1] += my_values[I, J].ToString();
                    if (J < my_cols - 1) result[I + 1] += ";";
                }
            }
            return result;
        }
        ...
        /// <para>First String contains the number of Rows ; Columns</para>
        /// </summary>
        /// <param name="format">specifies the format of each number in the strings.</param>
        public string[] ToRowStrings(string format)
        {
            string[] result = new string[my_rows + 1];
            result[0] = my_rows.ToString() + ";" + my_cols.ToString();
            for (int I = 0; I < my_rows; I++)
            {
                for (int J = 0; J < my_cols; J++)
                {
                    result[I + 1] += my_values[I, J].ToString(format);
                    if (J < my_cols - 1) result[I + 1] += ";";
                }
            }
            return result;
        ...
        /// <summary>
        /// Creates a Matrix from an array of strings.
        /// <para>The first string must contain the number of Rows ; Columns</para>
        /// <para>The remaining strings must contain the ; separated numbers of a row.</para>
        /// </summary>
        /// <param name="rowstrings">Array of strings. Array length = number of rows + 1.</param>
        /// <returns></returns>
        public static Matrix FromRowStrings(string[] rowstrings)
        {
            string[] sizedata = rowstrings[0].Split(';');
            string[] valuedata;
            int rows = int.Parse(sizedata[0]);
            int cols = int.Parse(sizedata[1]);
            Matrix result = new Matrix(rows, cols);
            for (int I = 0; I < rows; I++)
            {
                valuedata = rowstrings[I + 1].Split(';');
                if (valuedata.Length != cols)
                {
                    throw new Exception("Incorrect format of the string[].");
                }
                for (int J = 0; J < cols; J++)
                {
                    result.SetValue(I, J, double.Parse(valuedata[J]));
                }
            }
            return result;
        }
