...
namespace _3D_Cloth
{
    class Particle
    {
        ...
        public Particle(double mass, Point3D position, bool show)
        {
            my_Mass = mass;
            my_Position = new Vector3D(position.X, position.Y, position.Z);
        ...
        {
            get { return new Point3D(my_Position.X, my_Position.Y, my_Position.Z); }
            set 
            { 
                my_Position = new Vector3D(value.X, value.Y, value.Z);
